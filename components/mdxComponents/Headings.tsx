...
export default function H(props: { looksLike?: HeadingLevels; className?: string; id?: string; as?: HeadingLevels; children: React.ReactNode }) {
  ...
  return (
    <Tag {...forwardedProps} className={clsx(heading, className, looksLike || as || '')} id={id}>
      ...
      {id && <a className="hash-anchor" href={`#${id}`}><LinkIcon /></a>}
    </Tag>
  );
}
