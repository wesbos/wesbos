...
export function cloudflareWasm(pluginConfig: PluginConfig = {}): Plugin {
  return {
    ...
    async resolveId(source, importer) {
      ...
      assert(resolved, `Unexpected error: could not resolve Wasm module ${source}`);
    ...
    renderChunk(code, chunk) {
      ...
      while ((match = moduleRE.exec(code))) {
        ...
        const relativePath = normalizePath(path.relative(path.dirname(chunk.fileName), emittedFileName));
