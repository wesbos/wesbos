...
![add up all the numbers in the ordersTotal array](./670.png)

As you can see, that does work. However, is that the best way to add up a bunch of things?

No, it is not.

![incorrect way of adding up all the numbers](./671.png)
...
![visualizations pulled from google image search using map filter reduce keyword](./672.png)
...
![mozilla developer network (mdn) for the array reduce method](./673.png)
...
![tallyNumbers function showing current tally and current total](./674.png)
...
![passing a second argument to reduce method to specify our start counter](./675.png)
...
![detailed console ouput for the tallyNumbers function for visualization](./676.png)
...
![console showing the sum of numbers upto the current step and current step value](./677.png)
...
![accumulator start from 0 as we are passing 0 as starting counter](./678.png)
...
![looping and logging the items type in console](./680.png)
...
![outcome of shirt after looping as NaN](./681.png)
...
![total shirts in console](./682.png)
...
![result of calculation as total shirts, total pants and total socks in console](./683.png)
