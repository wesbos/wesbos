...
We need to edit the <code>HTML.tmLanguage</code> file that is located in <code>preferences</code> →  <code>browse packages</code> → <code>HTML</code>.

Open it up and look for a line that looks like the following. It should be around line 286.

<code><string>(?:^\s+)?(&lt;)((?i:script))\b(?![^&gt;]*/&gt;)</string></code>

Replace it with this:

<code><string>(?:^\s+)?(&lt;)((?i:script))\b(?![^&gt;]*/&gt;)(?!.*&lt;/script&gt;)</string></code>
