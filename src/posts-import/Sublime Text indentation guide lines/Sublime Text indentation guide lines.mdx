...
By default, Sublime text will give you indentation guide lines that are all the same colour. This is helpful but can become a little confusing when you get 4-5 levels deep. A class example I always find myself is below. Is the <code>.getting</code> div inside <code>.row</code> or is it a sibling?

<img src="ss-2013-07-24-at-4.02.52-PM.png" />

Well, an easy fix to this is to turn on <strong>"draw_active"</strong> setting for <strong>"indent_guide_options":</strong>

By Default, sublime already has <strong>"draw_normal"</strong> turn on for you, but we need to set it again so we do not overwrite it. You can put the code below in <code>Preferences</code> → <code>Settings - User</code>:
...
Right away, I'm able to visually see that <code>.getting</code> is nested inside <code>.row</code>
