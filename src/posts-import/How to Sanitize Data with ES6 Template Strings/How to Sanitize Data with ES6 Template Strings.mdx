...
These last <a href="https://wesbos.com/template-strings-html/">few</a> <a href="http://wesbos.com/tagged-template-literals/">examples</a> we've been creating HTML and inserting it right into the DOM. If you have any sort of security background and you're probably screaming, <em><strong>"Wes, you must sanitize that data before you put it into the DOM!!!"</strong></em>
...
But instead, what we can do is add a new <code>const</code> called <code>dirty</code>, because our string is dirty because it still has it's <code>onload</code>, or iFrames, or any of the other nasty stuff people are trying to do.

Now we'll use our library that we loaded, and it's called with <code>DOMPurify.sanitize</code>. We'll sanitize the dirty HTML:
...
Now when you refresh, you can see see that the <code>onload</code> has been stripped out, along with any other nasty stuff!