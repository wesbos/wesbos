...
![array forEach output in console](./619.png)
...
![placing debugger in the logTopping callback function](./620.png)
...
![paused status in debugger in the dialog box](./621.png)
...
![different ways to write forEach loop](./622.png)
...
![forEach mdn documentation for syntax](./623.png)
...
![console log of the loop with current looping value, index and whole topping array](./624.png)
...
![adding a log of originalArray[index] + 1 to access current looping value via index](./625.png)
...
![accessing the current looping value directly or via array index in console](./626.png)

If you scroll all the way to the bottom, you will see that when we get to the last one, the next index returns `undefined`.

![at the last index next element returns undefined](./627.png)
...
![removing the undefined based on a ternary check whether we have a value at the next index or not](./628.png)
...
![console log of the prevTopping and nextTopping by adding 1 in the index or subtracting 1 from it](./629.png)
...
![printing goodbye when we reach the end of array using length property of array](./630.png)
