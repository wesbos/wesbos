...
![mozilla developer docs for the array reduce](./684.png)
...
![A text example having 31 kb worth of text](./685.png)
...
![array having 15911 length letters in console](./686.png)
...
![result of match method applied on string 'a' with /a/ regex](./687.png)
...
![result of match method applied on string 'cat' with /a/ regex](./688.png)
...
![result of match method applied on everything with /[a-z]/ regex](./689.png)
...
![result of match method applied on everything with /[a-zA-Z]/ regex](./690.png)
...
![regexr.com website to find regex patterns](./691.png)
...
![case insensitive pattern matching with /i flag](./692.png)
...
![geeting numbers with insensitive pattern matching with /i flag ](./693.png)
...
![cannot read property k of undefined error](./694.png)

In Wes' example, `k` is the second character.

![k as the second character](./695.png)
...
![count of 1 next to each character](./696.png)
...
![ternary check for letters exists in the array, if does then the count of that character should be equal to the existing count plus one, forgot the equal sign as well](./697.png)
...
![result of occurrence of each character in array](./698.png)
...
![using Object.entries() method returning array with first item being the key and the second item being the count](./699.png)
...
![one single reduce function showing all the work we did](./701.png)
