...
![screenshot of console showing logs (hey! and 12) and editor showing code](/205.png)
...
![console showing logged value as undefined](/207.png)

If you try to log another variable that does not exist, you will get an error 👇

![console showing Uncaught ReferenceError: cool is not defined](/208.png)
...
It is important to note that **let** and **const** variables are not hoisted, unlike **var** variables. Hence, in the above example if you replace var with let or const, you would be presented with a `ReferenceError` saying - `age is not defined`
