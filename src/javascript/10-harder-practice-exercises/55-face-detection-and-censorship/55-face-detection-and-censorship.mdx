...
![checking browser for face detection support](./731.png)
...
![write typeof FaceDetector to see if your browser supports FaceDetector](./732.png)
...
![enabling experimental flags](./733.png)
...
![allowing camera on a url](./734.png)
...
![open face.html to ask the user to access their webcam](./735.png)
...
![type pwd to see where you are on your computer](./737.png)
...
![in vs code rigght click the folder and select open in terminal](./738.png)

That opens the terminal up in the built in terminal in VS Code.

![terminal opened in vs code](./739.png)
...
![checking the npm and node version from terminal](./740.png)
...
![node js and npm installation](./741.png)
...
![install the dependencies using npm install](./742.png)
...
![packages finished installation](./743.png)
...
![get rid of the warnings using filter -DevTools in console](./746.png)
...
![use shortcuts CMD + A and then CMD + Shift + L to get cursor on every single line](./747.png)
...
![uncaught reference error: populateVideo is not defined](./750.png)
...
![store as global varibale in console](./751.png)
...
![pending promise in console](./752.png)
...
![tab is using camera or microphone notification](./753.png)
...
![MediaStream object in console](./754.png)
...
![allow the camera in the popup](./755.png)
...
![Detected Face telling where eyes, nose and mouth are](./756.png)
...
![bounding box in FaceDetector](./760.png)
...
![bounding box top, left, width and height representation in console](./761.png)
...
![log of width, height, top and left as object](./762.png)
...
![draw a box onto the canvas element](./763.png)
...
![box moves around as we move our face](./764.png)
...
![bounding box with actual face](./765.png)
...
![bounding box border as yellow colored](./766.png)
...
![mozilla developers docs for CanvasRenderingContext2D.drawImage method](./767.png)
...
![adding display none css style to video](./768.png)
...
![tiny little wes at the top corner of square](./769.png)
...
![face spreading all over](./770.png)
...
![pixelated face](./771.png)

![pixelated face](./772.png)
...
![increase size of the face with size 1](./776.png)

![increase size of the face with size 2](./777.png)
...
![create range input for scaling](./778.png)

Let's make the defaults line up with what we set.  Add `value="1.35"` to the input range.

![setting the default values for scale](./779.png)
...
![nodelist length in the console](./780.png)
...
![log of slider in console as it is moved](./781.png)
