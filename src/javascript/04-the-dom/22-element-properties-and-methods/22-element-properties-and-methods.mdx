...
![screenshot showing how to select and log to the browser console a h2 element](./229.png)
...
![screenshot showing outputs in the browser console when using textContent and innerText on elements](./230.png)

We have a set of properties when working with HTML. If we were to log the `innerHTML` of the `h2`, you would see 👇

![browser console output for the selected h2 innerHTML](./231.png)

There is also `outerHTML`, which should include the `h2` tag and whitespace that goes inside of it.

![browser console output for the selected h2 outerHTML](./232.png)
...
![screenshot showing html output of the pizza emoji being added into the p element before the p element itself](./233.png)
...
![screenshot showing html output of the pizza emoji being added into the p element just before its first child ](./234.png)
...
![screenshot showing html output clearly indicating that the html element has two strings inside of it](./235.png)
...
![mdn website screenshot showing Element information page](./236.png)
