...
_👆 Aside: if you go to any website and in the console type in `window.location` or `location` it will return to you the website's url and a whole bunch of information, as demonstrated above._

It returns an object full of information like the current page you're on, the host name, what the protocol is, and, if we are on a specifc port, it would be showing us that info.
