...
![A rainbow gradient line on a canvas](./396.png)
...
The reason for that is because you can think of the HTML width and height values we have set via HTML attributes as the actual width of an image.. those values represent that actual height and width of the canvas element.

However, the values we set in CSS are the size that it should be on in the screen. The reason Wes halved the size via CSS is so that it won't look pixelated.
...
![inspecting a canvas element in dev tools](./398.png)
...
The shake button has a class of `shake` which we can use to grab it.
...
![console showing CanvasRenderingContext2D object](./399.png)
...
Next you have to put the drawer somewhere, meaning you have to put the dot somewhere because that will be the starting point of the etch-a-sketch.
...
If you log the width and the height, what will you get?
...
![Editor showing canvas height and width declarations using object destructing](./401.png)
...
![console showing Math.random() outputs](./402.png)
...
![screenshot of MDN documentation for Math.random](./403.png)
...
![console showing Math.random() outputs multiplied by 100](./404.png)

Then what you can do is run `Math.floor(Math.random() * 100))`, which should give us a number between 0 and 99.

![console showing Math.floor(Math.random() * 100)) output](./405.png)
...
![canvas showing a black square dot](./406.png)
...
![console showing the type arrow key pressed](./407.png)
...
![Editor showing object destructing](./408.png)
...
![canvas showing a vertical black line](./409.png)
...
![screenshot of https://mothereffinghsl.com homepage](./412.png)
...
![canvas showing a line with random colors](./413.png)
...
![Editor showing CSS code](./414.png)
...
![console showing multiple clearCanvas function calls](./415.png)
...
![inspecting canvas element in dev tools](./416.png)

You will see that you are listening for the animationend event every time we run it 👇

![dev tools showing animationend events attached to canvas element](./417.png)
...
![canvas showing a multi color snake like line](./419.png)

Now click the Shake! button.

![canvas showing a multi color snake like line with some area cleared](./420.png)
