...
If instead of re-using the `handleClick` function we wanted to create anonymous functions and pass it to both event listeners separately, that would still work,but if you think of scaling that up to a whole bunch of buttons, then you have to make sure you reference every one of the buttons which is not very **DRY** (**don't repeat yourself**).
...
_Note: Wes often places all the selectors at the top of the file instead of placing them throughout the code. For this example, we are going to keep it within the middle of the file. Both methods work, putting it at the top is just a personal preference of Wes'._
...
`forEach` function will give you an argument that is each of the individual buttons, and we can name it whatever we want.
