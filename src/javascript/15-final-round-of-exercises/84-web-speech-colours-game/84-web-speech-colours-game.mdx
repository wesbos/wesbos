...
![web speech colors game](./1372.png)
...
![open chrome browser and navigate to chrome://flgs and turn on the `Experimental Web Platform features`](./1373.png)
...
![style.css contains few styles that makes word popping up](./1374.png)
...
![window.SpeechRecognition is undefined as it is window.webkitSpeechRecognition](./1376.png)
...
![overide SpeechRecognition as window.SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition, then we can access it directly using SpeechRecognition](./1377.png)

Now within `start` let's check for the existance of a property called `SpeechRecognition` that is in the window.

![checking 'SpeechRecognition' in window returns true now](./1378.png)
...
![on logging the recognition, we see there is no method like addEventListener](./1379.png)
...
![prompt for using the microphone](./1380.png)
...
![once you do it, you start listening to your voice](./1381.png)
...
![logged SpeechRecognitionEvents are very similar to an event object](./1382.png)
...
![results property contains a few items that speech recognition thinks you said](./1383.png)
...
![sometimes SpeechRecognition event returns list, from which grab the last one](./1385.png)
...
![console will logs the things you speak](./1386.png)
...
![logging out all the colors in the conosle](./1387.png)
...
![colors sorted by length in the console](./1388.png)
...
![html code we are working with](./1389.png)
...
![uncaught type error - cannot read property map of undefined)
...
![refreshing the page will leave the console empty](./1391.png)
...
![displaying the color you said](./1393.png)
...
![all the colors in tiles](./1394.png)
...
![light colored text in the dark color and vice versa](./1395.png)

What should happen is the dark ones have a class of "dark" and the lighter colors should have no class appended.

![dark colored tiles have dark class while light colored tiles has false class](./1396.png)
...
![fix the light colored tiles adding false class](./1397.png)
...
![uncaught reference error - results is not defined](./1398.png)
...
![calling colrs.red returns hex, !!colors.red returns true and if the property does not exists returns false](./1399.png)

We could also use the `in` syntax and it would give us the same results.

![use of in operator to check a property exists in object](./1400.png)
...
![when a color is spoken, we cross the word out by selecting span to the corresponding color](./1401.png)
...
![speaking the color logs the color in console](./1402.png)
...
![backgroundColor is changing as the word is spoken](./red.gif)
