...
[https://youtu.be/iejQvQF-vZo](https://youtu.be/iejQvQF-vZo)
...
Now every time we hit **CMD + B** our coffeescript file will be compiled with the build script. If you want to watch the file for changes after the first compile, simply switch the first line to
`"cmd": ["coffee","-wc", "$file"],`. If anyone is using my [CoffeeScript-Growl plugin](https://github.com/wesbos/coffeescript-growl), change your first line to `"cmd": ["coffee", "-r", "coffeescript-growl", "-wc", "$file"],` to get growl notifications.
...
```js
{ "cmd": ["node", "$file"], "selector" : "source.js", "path" : "/usr/local/bin" }
```
...
[![](facebook-share2.png)](https://sublimetextbook.com/?utm_source=buildsystems&utm_medium=banner&utm_campaign=product)
