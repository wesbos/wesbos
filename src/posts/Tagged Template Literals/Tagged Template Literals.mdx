...
That means that we can run a template string through a function, and rather than have the browser immediately assign that value to a variable, we can have control over how this actual string is made.
...
For this, We can use **ES6 rest operator**, which is three dots. If we do `...values`, what it's going to do is take the <em>rest</em> of the values that are passed to that argument, and put them into an array for us.
...
The other things we get is an array of `values`, which we passed in inside the template literal with `${}`. We get `"Snickers"` and `"100"`, which are the values that I passed in.
...
Then, we want to loop over the first strings array here, tack on `My dog's name is `, and then the `first value`, and ` he is ` and the `second value`, and ` then years old` and then the `third value`, but in this case, we don't have a third value.
...
First of all, remember, that's because the `strings array` is **always going to be one larger than the values array**. When we hit that last one, it's only going to be a string and there's going to be no value to tack on the end. You could check if the value of `values[i]` is `undefined`, and if it is, then don't put it on.
