...
```js
const first = 'Wes'; const aboutMe = `I love to do evil![](http://unsplash.it/100/100?random)`; const html = `

### ${first}

${aboutMe} `; const bio = document.querySelector('.bio'); bio.innerHTML = html;
``` 
...
```html
<script>const first = 'Wes'; const aboutMe = `I love to do evil <img src="http://unsplash.it/100/100?random" onload="alert('you got hacked');" />`; const html = ` <h3>${first}</h3> ${aboutMe} `; const bio = document.querySelector('.bio'); bio.innerHTML = html;</script>
```
...
```js
function sanitize(strings, ...values) { return strings.reduce((prev, next, i) => `${prev}${next}${values[i] || ''}`); } const first = 'Wes'; const aboutMe = sanitize`I love to do evil ![](http://unsplash.it/100/100?random)`; const html = `
...
```js
function sanitize(strings, ...values) { const dirty = strings.reduce((prev, next, i) => `${prev}${next}${values[i] || ''}`); } const first = 'Wes'; const aboutMe = sanitize`I love to do evil ![](http://unsplash.it/100/100?random)`; const html = `
...
```js
function sanitize(strings, ...values) { const dirty = strings.reduce((prev, next, i) => `${prev}${next}${values[i] || ''}`, ''); return DOMPurify.sanitize(dirty); } const first = 'Wes'; const aboutMe = sanitize`I love to do evil ![](http://unsplash.it/100/100?random)`; const html = `
