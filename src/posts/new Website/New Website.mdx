...
My old site, built in 2014, was starting to feel a little creeky.

It was running on WordPress. I had over a decade on WordPress and I still love it, but Ill explain why I left WP in just a second. Here is the tech I used on the old site:
...
Many of you will ask why I didn't use [Next.js](https://nextjs.org/) - a framework of which I love and have built much of my course platform on. I used to say that Gatsby was for websites and Next.js was for apps. I don't belive that to be true as Next.js has per-page opt-in static generation and on-demand page builds in development. So, it comes down to:
...
Most of my blog posts were already written in Markdown, so I went wit MDX which allows you to use React components inside of your markdown. Handy for embedding Video players, graphs or anything else that is already a React component.
...
There are a few pieces of dynamic content on footer of this site - the latest Podcast, Tweets, Instagram stories and Instagram posts.
...
For this I use Amazon's Lamda functions. I loved this workflow of needing a "sip of backend", I just created [a new file in this folder](https://github.com/wesbos/wesbos/tree/master/functions)
...
I'm building and hosting it on [Netlify](https://www.netlify.com). I really like how it's hooked up to github and auto deploys.

I really wanted to maintain a viewable view-source, so after a tweet asking about nicely formatting the compacted HTML, James Vickery build this really cool [gatsby-plugin-prettier-build](https://github.com/jmsv/gatsby-plugin-prettier-build/) plugin that hooks into onPostBuild and runs the HTML through prettier.
...
Gatsby images is literally resizing and making multiple versions of each image, so that also takes some time.
...
I am currently running on the later, and big thanks to [Jason Lengstorf](https://twitter.com/jlengstorf) for helping me get this up and running. My build still take ~10 mins to build and deploy which is less than excellent, but I have hope for Gatsby incremental builds in the future.
...
**Gatsby queries** was another huge pain point. Only being able to have a dynamic query at a page level is weird. This means you can't do things like query an image unless you have it's filename as a string.
