...
test('it parses twitter and x.com links', () => {
  ...
  expect(parseSocialLink(`https://x.com/wesbos/status/1834242925401694490`)).toMatchObject({
    ...twitterOutput,
    url: 'https://x.com/wesbos/status/1834242925401694490',
  });
  expect(parseSocialLink(`https://twitter.com/wesbos/status/1834242925401694490`)).toMatchObject(twitterOutput);
  expect(parseSocialLink(`https://twitter.com/wesbos/status/1834242925401694490/`)).toMatchObject(twitterOutput);
  expect(parseSocialLink(`https://twitter.com/wesbos/status/1834242925401694490?`)).toMatchObject(twitterOutput);
  expect(parseSocialLink(`https://twitter.com/wesbos/status/1834242925401694490?ref_src=LOL`)).toMatchObject(
    twitterOutput,
  );
});
...
test('it parses tiktok links', () => {
  ...
  expect(
    parseSocialLink(
      `https://www.tiktok.com/@wesbos/video/7444990056906919223?is_from_webapp=1&sender_device=pc&web_id=7365956764889007621`,
    ),
  ).toMatchObject({
    type: 'tiktok',
    handle: 'wesbos',
